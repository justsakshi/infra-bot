...
| 0 | 09:45 daily | `key_health_monitor.py` | Probes every Smartlead API key 15 min before the sync. A rotated/revoked key is otherwise SILENT — every job catches the 401, logs a line nobody reads, and the client simply disappears from sheets and monitors (real incident 2026-07-10: one account's key rotated 3× in a day, another was dead on Render for over a day). Exit 2 = dead key, 1 = unreachable, 0 = all valid; alerts Slack when a channel is set | Log `[key-health] all keys valid`, or `🚨 DEAD API KEY(S)`; Mongo `key_health_checks` | ✅ READY — read-only, no enable flag |
| 6d | hourly 9-21 at :15 | `nc_test_executor.py` | Non-connected (Anjali-style) placement tests via the "NC Tests" sheet tab. Human creates the non-connected test in the PL UI (names it the inbox email OR its domain) and pastes seed list + Track-ID into the tab; the robot sends the test email from the inbox via a one-off campaign in the inbox's own account, polls the PL test, writes results to Mongo + API Tests tab, restores the inbox's daily limit, pauses the campaign. Also pure-ingests tests the human already sent manually (matched by name/domain) — zero credits, zero process change | Row moves blank → SENT → DONE in "NC Tests"; result row appears in "API Tests"; log `[nc-test]` | ⏸ DRY-RUN (`NC_TEST_ENABLED=false`) — NEW 2026-07-10, ingestion path proven live on Anjali's test 475632 |
