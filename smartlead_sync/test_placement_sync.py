...
# --- the coverage gate is the whole point ---
thin = summarise(535281, report(coverage=0.49, inbox_pct=88.1), senders(("a@x.com", 7, 8)))
ok(thin["scored"] is False,
   f"49% coverage is not a verdict (gate {PLACEMENT_MIN_COVERAGE})")
ok(thin["inbox_pct"] == 88.1,
   "the thin percentage is still recorded - we keep the reading, we just do not trust it")

full = summarise(535537, report(), senders(("a@x.com", 17, 20)))
ok(full["scored"] is True, "84% coverage clears the gate")

# --- per-domain rollup: mailboxes on one domain sum, they do not average ---
row = summarise(535537, report(), senders(
