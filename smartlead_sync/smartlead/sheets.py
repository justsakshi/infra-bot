...
_HEADER_LABELS = {
    ...
    "Campaign Metrics": {
        "client": "Client",
        "campaign": "Campaign name", "platform": "Platform",
        "launch_date": "Campaign Launch Date", "status": "Campaign Status",
        ...
        "connections_sent_yesterday": "Connections sent yesterday",
        "connections_accepted": "Connections accepted",
        "msg_sent": "Msg Sent", "msg_sent_yesterday": "Msg sent yesterday",
        "positive_responses_yesterday": "Positive Responses Yesterday",
...
class SheetsWriter:
    ...
    MASTER_COLUMNS = [
        ...
        "capacity_left", "true_load", "available_capacity", "demand",
    ...
    def _write_glossary_tab(self) -> None:
        ...
        rows: list[list[str]] = [
            ...
            ["", "Demand/Day", "What the inbox's ACTIVE campaigns ask of it per day: each campaign's daily limit / its inbox count, summed. Paused, completed and archived campaigns are not counted.", "Above Max/Day = oversubscribed: the campaigns want more than the mailbox can send"],
            ["", "True Load", "Demand/Day capped at the mailbox's real ceiling (the smaller of Max/Day and the provider cap: Outlook 10, Gmail 25, SMTP 15). Campaigns compete for a mailbox's sends; they do not stack.", "Never above Max/Day"],
            ["", "Avail. Capacity", "THE NUMBER TO ATTACH BY: sends/day this inbox can still take = ceiling - True Load, and 0 while warming. To staff a campaign, take FREE inboxes highest Avail. Capacity first until the sum reaches the campaign's daily limit; if it cannot, the limit is too high for the free fleet.", "0 = full; ties: fewer campaigns, lower bounce first"],
